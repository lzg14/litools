...
| **🔍 搜索** | 多关键词高亮 / 命中行底色 + 滚动条定位标记 / 命中计数与上下导航 / 替换 / 过滤 / 流式搜索大文件（50MB+） |
| **◎ 正则** | 正则测试（行号 + 实时高亮）/ 正则解释 / 常用正则速查（折叠面板，单击填入测试） |
| **⏱ 时间戳** | 秒/毫秒/微秒互转（自动识别）/ 相对时间 |
| **⏱ 计时器** | 秒表 / 计次 / 重置 |
| **⏳ 定时器** | 倒计时（快捷时长 / 自定义时分秒）/ 暂停 / 重置 / 到点提醒 |
| **🗄️ SQL** | IN 语句生成 / INSERT 解析（列值对应，支持转义与批量）/ SQL 格式化 |
| **🔐 编解码** | Base64 / URL / HTML 实体 / 进制转换 / 二进制文本 |
| **📑 Diff** | 文本差异对比 / 交换 / 高亮显示 |
| **🎲 生成** | UUID（多种格式） / Hash（SHA-1/256/384/512） / 颜色 / 密码 / Cron 解析 |
